...
public class Triangle 
{
	...
	public static void drawLine(BufferedImage buff, DepthBuffer depthBuff, Point3D p1, Point3D p2)
	{
		float dx = p2.x - p1.x;
		float dy = p2.y - p1.y;
		float dz = p2.z - p1.z;
		...
		if(pk.y>=0 && pk.y<buff.getHeight() && pk.x>=0 && pk.x<buff.getWidth() && pk.z > depthBuff.depthBuff[(int)pk.x][(int)pk.y]) {
		...
		if (p1.x == p2.x) {	 // Line is vertical
			for (int k = 0; k < steps; k++) {
				...
				if(y>=0 && y<buff.getHeight() && x>=0 && x<buff.getWidth() && z > depthBuff.depthBuff[(int)pk.x][(int)pk.y]) {
					depthBuff.depthBuff[(int)pk.x][(int)pk.y] = pk.z;
	...
	public static void drawTriangle(BufferedImage buff, DepthBuffer depthBuff, Point3D p1, Point3D p2,
			Point3D p3, boolean do_smooth) {
